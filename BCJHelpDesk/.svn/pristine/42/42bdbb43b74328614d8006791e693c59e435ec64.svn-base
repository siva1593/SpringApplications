...
/**
 * It implements methods and maps the requests related to
...
public class TicketController {
	...
	/**
	 * Map's the request to retrieve tickets of student/consultant.
	 * 
	...
	/**
	 * It Forwards list of tickets for a requested user.
	 * 
	 * @param session
	 *            To get user Id for the session
	...
	/**
	 * It map's request to create new ticket and forwards category for
	 * populating data in the view.
	 * 
	...
	/**
	 * It forwards sub category list based on the category selected by user.
	 * 
	 * @param request
	 * @param model
	 *            It has category value information.
	 ...
	 * 
	 * @param model
	 *            Gets the ticket's data to be approved.
	 * @param ticket
	 *            The model data is mapped with the ticket entity class.
	 * @param session
	 *            To get the user Id.
	 * @return return message and view.
	 */
	@RequestMapping(value = "/submitTicket", method = RequestMethod.POST)
	public String createTicket(Ticket ticket, Model model, HttpSession session) {
		logger.info("In controller class, saveTicket method");

		int userId = (Integer) session.getAttribute("userId");
		logger.info("User Id in ticket method " + userId);

		if (ticketService.createTicket(ticket, userId)) {
			return "UserTickets";
		} else {
			return "UserHome";
		}
